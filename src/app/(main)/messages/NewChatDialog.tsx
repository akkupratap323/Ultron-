...
export default function NewChatDialog({
  ...
  const { data, isFetching, isError, isSuccess } = useQuery({
    ...
    enabled: !!client && !!loggedInUser?.id,
  });
  ...
  return (
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent className="bg-card p-0" aria-describedby="new-chat-description">
        ...
        <div>
          ...
          {!!selectedUsers.length && (
            <div className="mt-4 flex flex-wrap gap-2 p-2" role="list" aria-label="Selected users">
          ...
          <hr />
          <div className="h-96 overflow-y-auto" role="listbox" aria-label="User search results">
            {isSuccess &&
              data?.users?.map((user) => (
                <UserResult
                  key={user.id}
            ...
            {isFetching && (
              <div className="flex justify-center my-3">
                <Loader2 className="animate-spin" aria-label="Loading users" />
              </div>
            )}
            {isError && (
        ...
        <DialogFooter className="px-6 pb-6">
          <LoadingButton
            ...
            aria-label={`Start chat with ${selectedUsers.length} selected user${selectedUsers.length !== 1 ? 's' : ''}`}
          >
            Start chat
          </LoadingButton>
        </DialogFooter>
...
function UserResult({ user, selected, onClick }: UserResultProps) {
  return (
    <button
      ...
      aria-label={`${selected ? 'Deselect' : 'Select'} ${user.name} (@${user.username})`}
    >
      ...
      {selected && <Check className="size-5 text-green-500" aria-hidden="true" />}
    </button>
  );
}
...
function SelectedUserTag({ user, onRemove }: SelectedUserTagProps) {
  return (
    ...
    >
      ...
      <p className="font-bold">{user.name}</p>
      <X className="mx-2 size-5 text-muted-foreground" aria-hidden="true" />
    </button>
  );
}
